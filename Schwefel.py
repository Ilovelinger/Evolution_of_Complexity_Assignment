...
def Schwefel(individual):
    ...
    for k in range(len(decimalnumberList)):
        scalenumber = -500.0 + ((500.0 + 500.0) / 2 ** 16 * decimalnumberList[k])
        scalenumberList.append(scalenumber)
    part = 0
    for i in range(len(scalenumberList)):
        part += scalenumberList[i] * np.math.sin(abs(scalenumberList[i]) ** 0.5)
    result = 418.9829 * 10 - part
    return result
...
class Member():
    ...
    def __init__(self, bitsString=None):
        ...
        self.fitness = Schwefel(bitsString)
...
def Mutation(parent, n):
    ...
    child.fitness = Schwefel(child.bitsString)
    return child
...
def twoPointCrossover(x, y, n):
    ...
    newchild1.fitness = Schwefel(child1)
    newchild2 = Member(child2)
    newchild2.fitness = Schwefel(child2)
    ...
    for i in range(len(population)):
        for j in range(len(population[i])):
            ...
            while k != len(population):
                ...
                k = k + 1
            member.fitness = Schwefel(integralIndividual)
    ...
    while generation != iteration:
        ...
        for i in range(n):
            ...
            while k != len(population):
                ...
                k = k + 1
            bestMutation.fitness = Schwefel(integralIndividual)
