...
def Rastrigin(individual):
    ...
    for k in range(len(decimalnumberList)):
        scalenumber = -5.12 + (5.12 + 5.12) / 2 ** 16 * decimalnumberList[k]
        scalenumberList.append(scalenumber)
    part = 0
    for i in range(len(scalenumberList)):
        part += scalenumberList[i] ** 2 - 3.0 * np.math.cos(2 * np.pi * scalenumberList[i])
    result = 3.0 * 20 + part
    return result
...
class Member():
    ...
    def __init__(self, bitsString=None):
        ...
        self.fitness = Rastrigin(bitsString)
    ...
    for i in range(len(population)):
        for j in range(len(population[i])):
            ...
            while k != len(population):
                ...
                k = k + 1
            member.fitness = Rastrigin(integralIndividual)
    ...
    while generation != iteration:
        ...
        for i in range(n):
            ...
            while k != len(population):
                ...
                k = k + 1
            bestMutation.fitness = Rastrigin(integralIndividual)
